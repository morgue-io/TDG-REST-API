    ...
    try {
        if (req.headers.authorization) {
            jwt.verify(req.headers.authorization.split(' ')[1], process.env.JWT_SECRET, async (err, userObj) => {
                ...
                } else if (await rtokenModel.findOne({ email: userObj.email, utype: 'employee' }) != null) {
                    req.IS_AUTH = true;
                    req.USEROBJ = await employeeModel.findOne({ _id: userObj._id });
                    if (!req.USEROBJ.meta.is_activated)
...
exports.refreshAuthorisationHandler = async (req, res) => {
    try {
        if (req.headers.authorization) {
            ...
            const userObj = await employeeModel.findOne({ email: rtokenObj.email });
